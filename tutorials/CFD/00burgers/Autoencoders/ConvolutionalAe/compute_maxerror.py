...
snap_true_frame = snapshots_true.reshape(-1, 3, domain_size, domain_size)[:, :2, :, :]
snap_true_vec = snap_true_frame.reshape(-1, 2*domain_size**2)
...
snap_proj_frame = err_snap_proj_true.reshape(-1, 2, domain_size, domain_size)
# plot_snapshot(snap_proj_frame, 1000)

# rel L2
err_abs = np.abs(snap_proj_frame-snap_true_frame)
err_vec = err_abs.reshape(-1, 2*domain_size**2)
...
error_proj_rel = error_proj/norm
print("relative projection error shape", error_proj.shape)
np.save("./errLinfUconvAeProjection.npy", error_proj_rel)

...
def main(args):
    ...
    # load deim indices
    deim_idx = np.load(WM_PROJECT + "deim_indeces.npy")
    deim_idx = np.vstack((deim_idx, deim_idx)).squeeze()
    print("deim indeces shape: ", deim_idx.shape)

    # load residual fields
    PtrList2Eigen
    ...
    ## GPR
    mask = deim_idx ==1
    snap_rec = snap_rec[:, mask]
    print("restricted snap rec: ", snap_rec.shape)
    nl_red_coeff = nl_red_coeff.detach().cpu().numpy()
    snap_rec = snap_rec.detach().cpu().numpy()
    plt.plot(range(n_total), snap_rec[:, 0])
    plt.plot(range(n_total), snap_rec[:, 1])
    plt.plot(range(n_total), snap_rec[:, 2])
    plt.show()
    ...
    train_snap, val_torch = torch.utils.data.random_split(
        nl_red_coeff.to(device), [n_train, n_total - n_train])

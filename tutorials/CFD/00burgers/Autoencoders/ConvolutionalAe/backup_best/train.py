...
def main(args):
    ...
    # print(nor.rescale(snaps_torch.to(device),
    # device).detach().cpu().numpy()[2500, 0, 0, 0],
    # nor.rescale(snaps_torch.to(device), device).detach().cpu().numpy()[3000,
    # 0, 0, 59]) plot_snapshot(nor.rescale(snaps_torch.to(device),
    # device).detach().cpu().numpy(), 4000)
    ...
    np.save(WM_PROJECT + "npTrueSnapshots_framed.npy",
            nor.vectorize2d(snap_true_scaled))
    print("saved snapshots for consistency test libtorch",
          nor.vectorize2d(snap_true_scaled).shape)
    ...
    print("Min max after scaling: ", np.min(snap_true_scaled),
          np.max(snap_true_scaled))
    ...
    if SAVE is True:
        ...
        # reproduce in pytorch inputs_torch =
        # torch.from_numpy(initial)#.transpose(0, 1) print("inputs shape",
        # inputs_torch.shape) inputs_repeated = inputs_torch.repeat(3600,
        # 1).requires_grad_(True) print("inputs shape", inputs_repeated.shape)
        # grad_output = torch.eye(7200).to(device, dtype=torch.float)
        # print("ATTE", grad_output.type()) decoder =
        # torch.jit.load("./decoder_gpu_4.pt") output1 =
        # model.decoder(inputs_repeated.to(device, dtype=torch.float)) output =
        # decoder(inputs_repeated.to(device, dtype=torch.float))
        print("saved")
        sys.exit()
    ...
    try:
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            for snap in train_loader:
                ...
                outputs = model(snap)
                loss = criterion(nor.frame2d(outputs), nor.rescale(
                    snap, device
                )) + regularizerl1(
                    model, device, factor=factor
                )  # + regularizerl2(model, device, factor=0.0005)#+ torch.norm(nor.frame2d(outputs)-nor.rescale(snap, device), p=ORDER)
            ...
            if epoch % args.iter == 0:
                ...
                # validation plot where validation loss is worst index_list =
                # torch.sort(index, descending=True)[1]
                plot_two(nor.frame2d(outputs_val),
                         nor.frame2d(outputs_val) - nor.rescale(val_np),
                         index,
                         epoch,
                         title="validation")
                ...
                if loss_val < best:
                    ...
                    is_best = False
                    it += 1
                    if (loss_val<0.01 or epoch>200) and it > 100 and optimizer.param_groups[0]['lr']>2e-5:
                        optimizer.param_groups[0]['lr'] *= 0.5
                        print("LR CHANGED: ", optimizer.param_groups[0]['lr'])
                        it = 0
                    else:
                        if it > 10 and epoch > 10 and optimizer.param_groups[0]['lr']>2e-5:
                            optimizer.param_groups[0]['lr'] *= 0.5
                            print("LR CHANGED: ", optimizer.param_groups[0]['lr'])
                            it = 0

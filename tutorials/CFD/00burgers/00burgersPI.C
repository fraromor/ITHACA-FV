...
void nonlinear_one_parameter_initial_velocity_PI_training(tutorial00);
...
int main(int argc, char *argv[])
{
    ...
    else if (std::strcmp(argv[1], "pitrain") == 0)
    {
        // NM-LSPG-HR
        nonlinear_one_parameter_initial_velocity_PI_training(example);
    }
    else
    {
...
void nonlinear_one_parameter_initial_velocity_PI_training(tutorial00 test_FOM)
{
 // Read parameters from ITHACAdict file
    ITHACAparameters *para = ITHACAparameters::getInstance(test_FOM._mesh(),
                                                           test_FOM._runTime());
    ...
    // load the test samples
    Eigen::MatrixXd mu;
    test_FOM.mu = cnpy::load(mu, "parTrain.npy");
    ...
    // torch::Tensor train_snap_unscaled = ITHACAtorch::torch2Foam::ptrList2Torch(test_FOM.Ufield);

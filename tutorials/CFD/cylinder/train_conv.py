...
def plot_compare_spectra(snap,
    ...
    snap = snap.reshape(-1, DIM, 64**2)
    snap_rec = snap_rec.reshape(-1, DIM, 64**2)
    snap_error = snap_error.reshape(-1, DIM, 64**2)

    plt.plot(snap_rec[idx, idx_coord, :], label="Reconstructed")
    # plt.plot(snap_error[idx, :], label="max rel error")
    plt.plot(snap[idx, idx_coord, :], label="True", alpha=0.5)
    plt.legend()
...
def plot_spectra(snapshots):
    snap = snapshots.reshape(-1, DIM, 64**2)
    ...
    for i in range(2):
        for j in range(5):
            ix = next(idxs)
            axes[i, j].plot(snap[ix, 0, :], label="U0_" + str(ix))
...
def main(args):
    ...
    snapshots = np.load("snapUcompressed.npy").T.reshape(5001, 2, 64, 64)
    # modes = np.load("./modU.npy")[:, :64**2]
    # snapshots1 = snapshots_[:, :17466].dot(modes[:17466, :])
    # snapshots2 = snapshots_[:, 17466:].dot(modes[17466:, :])
    # snapshots = np.hstack((snapshots1, snapshots2))
    np.save("snapUcompressed.npy", snapshots)
    ...
    # print("modes shapes: ", modes.shape)
    # print("test snapshots shapes: ", test_snapshots.shape)

    # load eigenvalues
    eigU = load_evals("./Eigenvalues_U")[:64**2]
    ...
    min_sn = np.min(snapshots.reshape(-1, 2 * 64 * 64).T.reshape(2, -1),
                    axis=1)
    max_sn = np.max(snapshots.reshape(-1, 2 * 64 * 64).T.reshape(2, -1),
                    axis=1)
    ...
    min_sn_after = np.min(snapshots.reshape(-1, 2 * 64 * 64).T.reshape(2, -1),
                          axis=1)
    max_sn_after = np.max(snapshots.reshape(-1, 2 * 64 * 64).T.reshape(2, -1),
                          axis=1)
    ...
    train_snap, val_torch = torch.utils.data.random_split(
        torch.from_numpy(snapshots), [4500, n_snap - 4500])
    del snapshots
    ...
    try:
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            index_val = np.argmax(diff_val.reshape(
                -1, DIM * DOMAIN_SIZE**2)) // (DIM * DOMAIN_SIZE**2)

...
def plot_compare_spectra(snap,
    ...
    for sn in [snap, snap_rec, snap_error]:
        if len(sn.shape)==4:
            # print("sn.shape", sn.shape)
            sn = sn.reshape(-1, 4, 64**2)
        elif len(sn.shape)==2:
            sn = sn.reshape(-1, 1, 64**2)
...
def main(args):
    ...
    # linear scaling
    # print("lin", np.arange(snapshots.shape[0], 0, -1), np.arange(snapshots.shape[0], 0, -1).shape)
    ...
    max_sn_after = np.max(snapshots.reshape(-1, 4 * 64 * 64).T.reshape(4, -1),
                          axis=1)
    min_sn_test = np.min(test_snapshots.reshape(-1, 4 * 64 * 64).T.reshape(4, -1),
                         axis=1)
    max_sn_test = np.max(test_snapshots.reshape(-1, 4 * 64 * 64).T.reshape(4, -1),
                         axis=1)
    ...
    train_snap, val_torch = torch.utils.data.random_split(
        torch.from_numpy(snapshots), [9000, n_snap - 9000])
    ...
    snap_test_torch = torch.from_numpy(test_snapshots)
    test_np = snap_test_torch[:].detach().cpu().numpy()
    ...
    try:
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            index_val = np.argmax(diff_val.reshape(-1, DOMAIN_SIZE**
                                                   2)) // (DOMAIN_SIZE**2)
            ...
            del test_cuda
            diff_test = np.abs(outputs_test -
                               test_np) / max_sn_test[0].reshape(1, 1, 1)
            index_test = np.argmax(diff_test.reshape(-1, 4*DOMAIN_SIZE**2)) // (4*DOMAIN_SIZE**2)
            ...
            if epoch % args.iter == 0:
                ...
                if loss_val < best:
                    ...
                    is_best = False
                    it += 1
                    if (it > 200 and epoch > 300) or (it > 10 and epoch > 5
                                                      and epoch < 300):
